        ...
        <div class="py-4 px-4 rounded-bottom-4 flex-grow-1" style="background-color: #EFEDEF; overflow-y: auto;">
          ...
          <div class="row g-3 mx-3 px-4 pb-4 rounded-4" style="background-color: #EAE8EA;">
            <div class="d-flex align-items-center py-2 mb-md-2">
              ...
              <h1 class="fs-4 fw-bold mb-md-1 ms-3">BoQ Database</h1>
              <router-link
                :to="'/add-folder'"
            ...
            <div class="table-responsive px-2">
              <table class="table align-middle">
                ...
                <tbody>
                  <tr v-for="(folder, index) in folders" :key="folder._id">
                    ...
                    <td class="px-3">
                      <span :class="statusClass(folder.status)" class="px-md-0 px-3 py-1 fw-semibold d-flex justify-center align-items-center rounded-3">
                        {{ folder.status }}
                        <img :src="getStatusImage(folder.status)" class="ms-2" alt="status icon" style="width: 16px;">
                      </span>
                    </td>
                    <td class="px-3 d-flex justify-center">
                      <router-link :to="'/hardware/' + folder._id" class="btn rounded-3 fw-semibold px-3 py-1 me-2" style="background-color: #EFEDEF; border-color: #133E87; border-width: 0.150rem; color: #133E87;">View</router-link>
                      <router-link v-if="userRole === 'User' && folder.status == 'Pending'" :to="'/submit/' + folder._id" class="btn rounded-3 text-white fw-medium px-3 me-2" style="background-color: #133E87;">Submit</router-link>
                      <div v-if="userRole === 'Admin' && folder.status == 'On Review'" class="d-flex justify-center gap-2">
                        <div class="btn-group">
                          <button
                            v-if="folder.status !== 'Approved' && folder.status !== 'Rejected'"
                            type="button"

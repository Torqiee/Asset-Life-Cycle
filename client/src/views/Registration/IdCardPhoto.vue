    ...
    <div class="container d-flex justify-content-center align-items-center min-vh-100 min-vw-100" style="background-color: #ececec;">
      <div class="row border rounded-5 p-3 bg-white box-area justify-content-center">
        <div class="d-flex align-items-center pt-4 ps-4">
          ...
          <h2 class="fw-bolder fs-4 ps-4">Take a picture of your ID Card</h2>
        </div>
  ...
  export default {
    setup() {
      ...
      const captureOrContinue = async () => {
        if (captureMode.value) {
            // Capture logic similar to the first page
            ctx.value.save();
            ctx.value.scale(-1, 1);
            ctx.value.translate(-canvas.value.width, 0);
            ctx.value.drawImage(canvas.value, 0, 0, canvas.value.width, canvas.value.height);
            ctx.value.restore();
            ...
            registrationData.idCardImage = compressedImage; // Save as "idCardImage"
            ...
            router.push("/confirm-registration"); // Adjust to the next step in your flow
        }
        };
